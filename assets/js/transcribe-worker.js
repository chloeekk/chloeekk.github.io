...
self.addEventListener("message", async (event) => {
  ...
  try {
    if (message.type === "initialize") {
      ...
      if (!instance) throw new Error("语音模型初始化失败");
    ...
    if (message.type === "transcribe") {
      ...
      if (code !== 0) throw new Error(`语音识别启动失败（${code}）`);
    }
  } catch (error) {
    currentRun = null;
