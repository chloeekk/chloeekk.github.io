# 在Obsidian中高效管理图片的实用指南


做笔记用 Obsidian，难免会用到图片。直接复制粘贴图片进去确实方便，但久而久之你会发现图片摆放一团乱，调整大小也麻烦，想让图片居中更是难上加难。还有个坑，就是删了图片，文件却没真正消失，还藏在根目录里。

如果你还没有创建Vault或配置“文件与链接”选项，可以先完成[Obsidian安装与基础设置](https://chloevolution.com/zh-cn/posts/how-to-install-obsidian/)，再按照本文规划附件目录。
...
如果你经常把截图、会议照片或当天记录放进每日笔记，可以把这里的附件设置与[Obsidian Daily Notes工作流](https://chloevolution.com/zh-cn/posts/obsidian-daily-notes/)结合起来，避免每天的图片散落在Vault根目录。
...
网络图片虽然不占用Vault空间，但它始终依赖原网站。如果断开网络、原图片被删除，或者网站更换了图片地址，笔记中的图片就可能无法显示。

这个问题在使用[Obsidian Web Clipper](https://chloevolution.com/zh-cn/posts/obsidian-web-clipper/)时尤其常见。Web Clipper默认不会下载网页中的图片，而是在Markdown笔记中保留原网站的图片URL。刚剪藏时看起来没有区别，真正离线打开笔记时才会发现图片并不在自己的Vault里。

如果希望长期保存这些图片，可以使用Obsidian内置的`Download attachments for current file`命令。它会下载当前笔记中引用的网络图片，并将原来的外部链接替换为Vault中的内部附件引用。

操作步骤如下：

1. 打开包含网络图片的笔记。
2. 按`Ctrl/Cmd + P`打开命令面板。
3. 搜索`Download attachments for current file`。
4. 运行命令，并在出现的窗口中确认需要下载的图片。
5. 等待下载完成，再检查正文中的图片是否正常显示。


下载后的图片会进入Obsidian设置中指定的默认附件位置。如果还没有设置附件文件夹，建议先阅读下一节，将图片统一保存到`assets`或其他专用目录，避免它们直接散落在Vault根目录。

完成后，可以切换到Source Mode检查图片引用。原来的网络链接：
...
这个命令只处理当前打开的笔记，不会一次下载整个文件夹中的网络图片。如果有很多旧笔记需要处理，建议先从一篇测试笔记开始，确认附件位置和链接结果符合预期后，再逐篇执行。

部分网站会使用临时地址、权限验证或防盗链，下载时仍可能失败。运行命令后不要只看文件是否出现在附件目录，还要实际打开笔记，确认图片没有变成失效的内部链接。
...
最推荐的方法是使用专门的插件来管理附件文件。如果你还没有启用第三方插件，先阅读[Obsidian社区插件安装与安全指南](https://chloevolution.com/zh-cn/posts/obsidian-plugins/)，了解安装位置、更新方式和风险评估方法。
